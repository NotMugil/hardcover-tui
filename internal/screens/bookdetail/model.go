...
func NewFromUserBook(deps commands.Deps, ub *api.UserBook) *Model {
	s := spinner.New(
...
func NewFromID(deps commands.Deps, id int) *Model {
	s := spinner.New(
		spinner.WithSpinner(spinner.Dot),
		spinner.WithStyle(common.SpinnerStyle),
	)
...
func NewFromBookID(deps commands.Deps, bookID int) *Model {
	s := spinner.New(
		spinner.WithSpinner(spinner.Dot),
		spinner.WithStyle(common.SpinnerStyle),
	)
...
func NewFromListBook(deps commands.Deps, bookID int, listBooks []ListBookEntry, listIndex int, listID int, listName string) *Model {
	m := NewFromBookID(deps, bookID)
...
func (m *Model) Init() tea.Cmd {
	if m.loading {
		if m.loadByBook {
			return tea.Batch(m.spinner.Tick, commands.LoadBookByBookID(m.deps, m.bookID))
		}
		return tea.Batch(m.spinner.Tick, commands.LoadBookByPK(m.deps, m.bookID))
	}
	var cmds []tea.Cmd
	if m.userBook != nil {
		if m.userBook.Book.CoverURL() != "" {
			cmds = append(cmds, commands.LoadCover(m.userBook.Book.CoverURL(), 32, 16))
		}
		cmds = append(cmds, commands.LoadTags(m.deps, m.userBook.Book.ID))
		cmds = append(cmds, commands.LoadReviews(m.deps, m.userBook.Book.ID, 5))
	}
	if len(cmds) > 0 {
