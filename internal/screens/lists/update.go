...
func (m *Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case listSettledMsg:
		if msg.listID == m.pendingListID {
			m.booksLoading = true
			return m, tea.Batch(m.spinner.Tick, commands.LoadListBooks(m.deps, msg.listID))
		}
		return m, nil
	...
	case commands.UserListsLoadedMsg:
		...
		if len(m.lists) > 0 {
			m.booksLoading = true
			return m, commands.LoadListBooks(m.deps, m.lists[0].ID)
		}
		return m, nil

	case commands.ListBooksLoadedMsg:
	...
	case commands.ListCreatedMsg:
		...
		return m, tea.Batch(commands.LoadLists(m.deps, m.deps.User.ID), components.NotifyCmd(components.NotifySuccess, "List created"))

	case commands.ListDeletedMsg:
		m.loading = false
		if msg.Err != nil {
			m.err = msg.Err
			return m, components.NotifyCmd(components.NotifyError, msg.Err.Error())
		}
		return m, tea.Batch(commands.LoadLists(m.deps, m.deps.User.ID), components.NotifyCmd(components.NotifySuccess, "List deleted"))
	...
	case commands.BookAddedToListMsg:
		...
		if item, ok := m.list.SelectedItem().(listItem); ok {
			m.booksLoading = true
			return m, tea.Batch(m.spinner.Tick, commands.LoadListBooks(m.deps, item.data.ID), components.NotifyCmd(components.NotifySuccess, "Book added to list"))
		}
		return m, components.NotifyCmd(components.NotifySuccess, "Book added to list")
	...
	case commands.BookRemovedFromListMsg:
		...
		if item, ok := m.list.SelectedItem().(listItem); ok {
			m.booksLoading = true
			return m, tea.Batch(m.spinner.Tick, commands.LoadListBooks(m.deps, item.data.ID), components.NotifyCmd(components.NotifySuccess, "Book removed from list"))
		}
		return m, components.NotifyCmd(components.NotifySuccess, "Book removed from list")

	case commands.PrivacyUpdatedMsg:
		if msg.Err != nil {
			m.err = msg.Err
			return m, components.NotifyCmd(components.NotifyError, msg.Err.Error())
		}
		return m, tea.Batch(commands.LoadLists(m.deps, m.deps.User.ID), components.NotifyCmd(components.NotifySuccess, "Privacy updated"))
	...
	case tea.KeyMsg:
		if m.mode == modeCreate {
			...
			case "enter":
				...
				m.loading = true
				return m, tea.Batch(m.spinner.Tick, commands.CreateList(m.deps, name))
			case "esc":
				m.mode = modeNormal
				m.nameInput.SetValue("")
				m.nameInput.Blur()
			...
			case "enter":
				if m.privacyCursor >= 0 && m.privacyCursor < len(privacyOptions) {
					...
					if item, ok := m.list.SelectedItem().(listItem); ok {
						...
						m.mode = modeNormal
						return m, commands.UpdateListPrivacy(m.deps, l.ID, l.Name, desc, selected)
					}
				}
			}
			return m, nil
				...
				if len(m.searchResults) > 0 {
					...
					if cursor >= 0 && cursor < len(m.searchResults) {
						...
						if item, ok := m.list.SelectedItem().(listItem); ok {
							m.addSuccess = false
							m.addErr = nil
							return m, commands.AddBookToList(m.deps, item.data.ID, book.ID, item.data.Name)
						}
					}
				}
				return m, nil
		...
		if m.mode == modeConfirm {
			...
			if !m.confirm.Active {
				...
				if confirmed {
					...
					case "delete-list":
						m.loading = true
						return m, tea.Batch(m.spinner.Tick, commands.DeleteList(m.deps, m.confirmItemID))
					case "remove-book":
						return m, commands.RemoveListBook(m.deps, m.confirmItemID)
					}
				}
			}
			return m, nil
