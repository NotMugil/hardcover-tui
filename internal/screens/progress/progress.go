...
func New(deps commands.Deps, ub *api.UserBook) *Model {
	ti := textinput.New()
...
func (m *Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	switch msg := msg.(type) {
	case commands.ProgressUpdatedMsg:
		m.loading = false
		if msg.Err != nil {
			m.err = msg.Err
		...
		m.success = true
		return m, tea.Batch(
			components.NotifyCmd(components.NotifySuccess, "Progress updated"),
	...
	case tea.KeyMsg:
		...
		if m.confirming {
			...
			case "y", "Y":
				...
				m.loading = true
				m.err = nil
				readID := m.userBook.UserBookReads[0].ID
				return m, tea.Batch(m.spinner.Tick, commands.UpdateProgressWithDates(m.deps, readID, m.pendingPages, m.pendingStarted, m.pendingFinished))
			case "n", "N", "esc":
				m.confirming = false
				return m, nil
			}
