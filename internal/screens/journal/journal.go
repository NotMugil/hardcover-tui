...
func (m *Model) Init() tea.Cmd {
	return tea.Batch(m.spinner.Tick, commands.LoadJournals(m.deps, m.deps.User.ID, 20))
}
...
func (m *Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case commands.JournalSavedMsg:
		...
		m.success = true
		m.textarea.SetValue("")
		return m, commands.LoadJournals(m.deps, m.deps.User.ID, 20)
	...
	case tea.KeyMsg:
		...
		case "d":
			if item, ok := m.list.SelectedItem().(journalItem); ok {
				m.loading = true
				return m, tea.Batch(m.spinner.Tick, commands.DeleteJournalAndReload(m.deps, item.data.ID))
			}
		}
	}
