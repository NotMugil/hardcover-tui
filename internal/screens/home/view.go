...
func (m *Model) View() string {
	...
	name := m.deps.User.Username
	if m.deps.User.Name != nil && *m.deps.User.Name != "" {
		name = *m.deps.User.Name
	}

	var statsContent strings.Builder
	statsContent.WriteString(common.TitleStyle.Render(fmt.Sprintf("@%s", m.deps.User.Username)))
	if m.deps.User.Name != nil && *m.deps.User.Name != "" {
	...
	if m.deps.User.Flair != nil && *m.deps.User.Flair != "" {
		statsContent.WriteString("\n" + lipgloss.NewStyle().Foreground(common.ColorAccent).Bold(true).Render(*m.deps.User.Flair))
	}
	statsContent.WriteString("\n")
	statsContent.WriteString(fmt.Sprintf("Reads: %s\n",
		common.LabelStyle.Render(fmt.Sprintf("%d", m.deps.User.BooksCount)),
	))
	statsContent.WriteString(fmt.Sprintf("Followers: %s\n",
		common.LabelStyle.Render(fmt.Sprintf("%d", m.deps.User.FollowersCount)),
	))
	statsContent.WriteString(fmt.Sprintf("Following: %s",
		common.LabelStyle.Render(fmt.Sprintf("%d", m.deps.User.FollowedUsersCount)),
	))
