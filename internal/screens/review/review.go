...
func (m *Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case commands.ReviewSavedMsg:
		...
		m.success = true
		return m, components.NotifyCmd(components.NotifySuccess, "Review saved")
	...
	case tea.KeyMsg:
		...
		case "ctrl+s":
			...
			m.loading = true
			m.err = nil
			return m, tea.Batch(m.spinner.Tick, commands.SaveReview(m.deps, m.userBook.ID, review, false))
		case "esc":
			if m.editing {
				m.editing = false
				m.textarea.Blur()
